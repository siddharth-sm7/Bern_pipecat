...
void oai_audio_decode(uint8_t *data, size_t size) {
  ...
  if (decoded_size > 0) {
    ...
    if ((ret = esp_codec_dev_write(spk_codec_dev, decoder_buffer, decoded_size * sizeof(uint16_t))) != ESP_OK) {
...
void oai_send_audio(PeerConnection *peer_connection) {
  if (esp_codec_dev_read(mic_codec_dev, read_buffer, PCM_BUFFER_SIZE) != ESP_OK) {
  ...
  auto encoded_size = opus_encode(opus_encoder, (const opus_int16* )read_buffer, PCM_BUFFER_SIZE / sizeof(uint16_t), encoder_output_buffer, OPUS_BUFFER_SIZE);
